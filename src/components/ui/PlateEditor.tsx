...
function ToolbarButton({
  ...
  return (
    <button
      ...
      title={title}
      className={`
        inline-flex items-center justify-center h-8 w-8 rounded-md text-sm font-medium transition-colors duration-150
        ${active
...
function ToolbarDivider() {
  return <div className="h-5 w-px bg-gray-200 mx-1.5" />
}
...
function EditorToolbar({
  ...
  return (
    <div className="flex items-center gap-0.5 px-2 h-10 border-b border-gray-200 bg-white flex-wrap">
      {/* Marcas de texto */}
      <ToolbarButton
        active={isMark('bold')}
      ...
      <div
        className={`rounded-xl border overflow-hidden transition-all duration-150 ${
          readOnly
            ...
            : 'border-gray-200 hover:border-gray-300 focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/15'
        }`}
      >
        <Plate editor={editor} onChange={handleChange}>
          ...
          <PlateContent
            readOnly={readOnly}
            className={`plate-editor px-4 py-3 text-[15px] leading-relaxed min-h-[120px] outline-none ${readOnly ? 'text-gray-600' : 'text-gray-900'}`}
